  ...
  return (
    <div className="space-y-8 max-w-5xl mx-auto">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-heading font-extrabold tracking-tight text-white">Budgets</h1>
          <p className="text-body text-gray-400">Establish and monitor monthly category budgets.</p>
        </div>
        <button
          onClick={() => {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {activeBudgets.length === 0 ? (
          ...
          activeBudgets.map((b) => {
            ...
            return (
              <div key={b.id} className="glass-card p-6 rounded-2xl border-white/5 flex flex-col justify-between space-y-4">
                <div className="flex justify-between items-start">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-left">
                      <p className="text-caption font-bold text-white">{b.categoryName}</p>
                      <p className="text-micro text-gray-500">Monthly Budget</p>
                    </div>
                  </div>
                  <div className="flex gap-2">
                    <button
                      onClick={() => {
                        ...
                        setMonthlyLimit(b.monthlyBudget.toString());
                        setShowAddForm(true);
                      }}
                      className="p-2 rounded-lg bg-white/5 border border-white/5 text-gray-400 hover:text-white cursor-pointer active:scale-95"
                    >
                      <Edit2 className="w-3.5 h-3.5" />
                    </button>
                    <button
                      onClick={() => handleDeleteBudget(b.id)}
                      className="p-2 rounded-lg bg-red-950/20 border border-red-900/20 text-red-400 hover:bg-red-900/30 cursor-pointer active:scale-95"
                    >
                      Remove
                    </button>
                  </div>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between text-caption font-bold">
                    <span className="text-gray-400">
                      Spent: {user?.currency === 'INR' ? '₹' : '$'}{Math.round(b.spent).toLocaleString()}
                    </span>
                    <span className="text-white">
                      Limit: {user?.currency === 'INR' ? '₹' : '$'}{b.monthlyBudget.toLocaleString()}
                    </span>
                  </div>
